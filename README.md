...
Upstream app: [`CodeWithCJ/SparkyFitness`](https://github.com/CodeWithCJ/SparkyFitness) — images `codewithcj/sparkyfitness` (nginx frontend) and `codewithcj/sparkyfitness_server` (Node backend), version **1.7.1**, multi-arch `amd64` + `arm64`.
...
1. **Get the chart package** — download `sparkyfitness-0.0.7.tgz` from this repo's [Releases](https://github.com/abidals/SparkyFitness-Olares/releases/latest), or build it yourself:
   ```sh
...
2. **Upload and install** — no required prompts; the admin email, time zone and SMTP settings are pre-filled from your Olares profile:
   ```sh
   olares-cli market upload ./sparkyfitness-0.0.7.tgz
   olares-cli market install sparkyfitness -s upload --version 0.0.7 --watch
   ```
...
sparkyfitness-0.0.7.tgz   # pre-built chart package (what `market upload` consumes)
```
